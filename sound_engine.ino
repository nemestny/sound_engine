...
int frequency_min = 31; // частота  1 - 2000000 (Гц)  минимальная частота
int frequency_max = 150; //максимальная частота
int level_min = 0;   // частота ШИМ (0-255) минимальная громкость
int level_max = 200; // максимальная громкость
...
*/
  if (digitalRead(input)==HIGH){
      level +=5; //увеличение громкости
      frequency +=1; //увеличение частоты
      if (level > level_max) {
          level = level_max;
        }
 //     if (level == 255) {level = 257;}
      if (frequency > frequency_max){
          frequency = frequency_max;
        }
    } else {
      level -=5; //уменьшение громкости
      frequency -=1; //уменьшение частоты
      if (level < level_min) {
          level = level_min;
        }
